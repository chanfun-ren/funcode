...
using namespace cfs::rpc;
#define CLINETRETRYTIMES 4

class Client {
 private:
  std::vector<std::unique_ptr<MasterService::Stub>> cli_master_stubs_;
  std::map<std::string, std::unique_ptr<ChunkServerService::Stub>>
      cli_chunkserver_stub_map_;
  int client_id_;

 public:
  Client(std::vector<std::shared_ptr<Channel>> channels, int client_id)
      : client_id_(client_id) {
    for (auto ch : channels) {
      cli_master_stubs_.push_back(MasterService::NewStub(ch));
    }

    std::cout << "client_id: " << client_id_ << " init.";
  }

  // TODO:
  // 1. All APIs return value -> error code or error info.
  // 2. Get(), Put() have no consideration of data copies. done
 ...
 public:
  void TestFindAddr(const std::string &file_path);
 ...
 public:
  ...
  ChunkServerService::Stub *GetCliChunkServerStub(const std::string &addr);

  // GetFileInfo(): returns the file's path and the file size.
  // if return empty file'path means no this file.
  std::pair<std::string, std::uint64_t> GetFileInfo(
