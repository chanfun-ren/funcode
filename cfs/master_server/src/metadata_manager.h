...
class MetaDataManager {
 ...
 private:
  ...
  std::map<int64_t, std::vector<ChunkServerLocation>> handle_locations_map_;
 ...
 public:
  ...
  std::optional<std::vector<ChunkInfo>> GetWriteLocation(
      const std::string& file_path, size_t file_size);

 private:
  void SetNewFileMeta(std::string_view file_path, size_t file_size);
